...
assert.equal(sitemap.at(-1)?.url, routesById['article-when-to-aerate-lawn-iowa'].canonicalUrl)
...
console.log(`Task 25 Reviews validation passed: exact ownership, ${reviewRecords.length} governed/display-eligible records, ${HOME_REVIEW_LIMIT} Homepage records, bounded ${REVIEWS_INITIAL_COUNT}-item SSR, ${REVIEW_CATEGORIES.length} governed categories, rating/count restraint, and current 24-URL lifecycle.`)
