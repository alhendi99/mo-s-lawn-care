  ...
  return (
    <section aria-labelledby="ground-heading" className="bg-paper py-20 sm:py-28">
      <div className="mx-auto w-full max-w-[112rem] px-5 sm:px-8">
        <p className="eyebrow text-ink-soft">The part nobody photographs</p>
        <h2 id="ground-heading" className="display-lg mt-5 max-w-[20ch]">
          Good yards start
          <br />
        ...
        <div className="mt-14 sm:mt-20">
          <div className="flex items-baseline justify-between gap-4">
            <h3 className="eyebrow text-ink">Above the surface</h3>
            <span className="eyebrow text-ink-soft">What you see</span>
          </div>
          ...
          <ul className="mt-6 grid grid-cols-2 gap-x-4 sm:grid-cols-4 lg:grid-cols-8">
            {aboveGround.map((name, i) => (
              <li key={name} className="flex min-w-0 flex-col">
                <span className="font-display text-[0.8125rem] leading-tight font-semibold tracking-[-0.01em] uppercase sm:text-sm">
                  {name}
                </span>
                <span
                  aria-hidden="true"
        ...
        <div className="relative">
          ...
          <span className="absolute -top-[1.25rem] right-0 bg-paper pl-3 text-[0.5625rem] font-semibold tracking-[0.22em] text-ink-soft uppercase">
            Grade level
          </span>
        </div>
        ...
        <div className="relative overflow-hidden bg-evergreen px-5 pt-8 pb-10 sm:px-8">
          ...
          <div className="relative">
            <div className="flex items-baseline justify-between gap-4">
              <h3 className="eyebrow text-paper">Below / at the ground</h3>
              <span className="eyebrow text-paper/45">What actually decides it</span>
            </div>
            ...
            <ul className="mt-8 grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-4">
              {belowGround.map((name, i) => (
                <li key={name} className="flex min-w-0 flex-col">
                  ...
                  />
                  <span className="font-display text-[0.8125rem] leading-tight font-semibold tracking-[-0.01em] text-paper uppercase sm:text-sm">
                    {name}
                  </span>
                </li>
              ))}
            ...
            <p className="mt-10 max-w-prose text-[0.9375rem] leading-relaxed text-paper/70">
              Mowing keeps a property looking maintained. Aeration, seeding, feeding and grading are
              what make it stay that way — the work under the grass is what the neighbors never see.
            </p>
          </div>
        </div>
      </div>
