...
assert.equal(sitemap.at(-1)?.url, routesById['article-best-time-to-overseed-lawn-iowa'].canonicalUrl)
...
console.log(`Task 25 Reviews validation passed: exact ownership, ${reviewRecords.length} governed/display-eligible records, ${HOME_REVIEW_LIMIT} Homepage records, bounded ${REVIEWS_INITIAL_COUNT}-item SSR, ${REVIEW_CATEGORIES.length} governed categories, rating/count restraint, and current 25-URL lifecycle.`)
