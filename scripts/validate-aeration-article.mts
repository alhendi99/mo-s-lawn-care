...
assert.deepEqual(published, [article])
...
const futureArticles = blogArticles.filter(({ slug }) => slug !== article.slug)
assert.equal(futureArticles.length, 5)
...
assert.deepEqual(itemList.itemListElement, [{
  ...
  position: 1,
  name: article.h1,
  url: ownershipRoute.canonicalUrl,
}])
...
console.log('Task 28 aeration article validation passed: exact ownership, six sourced claim groups, three editorial gates, one published article, truthful schema, five isolated future 404 owners, and exact 24-URL lifecycle.')
