...
assert.match(collectionClientSource, /px-1 py-8 md:px-7 md:nth-\[2n\]:border-l/)
assert.doesNotMatch(collectionClientSource, /nth-\[[^\]]+\]:pl-0/)
...
console.log(`Task 25 Reviews validation passed: exact ownership, ${reviewRecords.length} governed/display-eligible records, ${HOME_REVIEW_LIMIT} Homepage records, bounded ${REVIEWS_INITIAL_COUNT}-item SSR, ${REVIEW_CATEGORIES.length} governed categories, rating/count restraint, and current 29-URL lifecycle.`)
