...
function SurfaceStage() {
  return (
    <div className="yard-xray__surface relative isolate overflow-hidden bg-paper">
      <div className="flex items-start justify-between gap-5 px-4 pt-5 sm:px-6 sm:pt-7 lg:px-8">
      ...
      <div className="grid items-end gap-6 px-4 pt-9 pb-10 sm:px-6 sm:pt-12 sm:pb-12 lg:grid-cols-[minmax(0,1fr)_22rem] lg:px-8">
        <h3 className="max-w-[12ch] font-display text-[clamp(2.65rem,6.6vw,7.2rem)] leading-[0.84] font-semibold tracking-[-0.065em] text-ink uppercase">
          One yard.
        ...
        <div className="border-l border-ink/15 pl-5">
          <p className="text-sm leading-relaxed text-ink/62 sm:text-base">
            The property changes every few months. The standard never does.
          </p>
          <p className="mt-4 font-mono text-[0.5625rem] tracking-[0.16em] text-ink-soft uppercase">
            <span className="md:hidden">Swipe through the year →</span>
            <span className="hidden md:inline">Hover a season to explore</span>
          </p>
        </div>
      </div>

      <div className="yard-xray__panorama overflow-x-auto overscroll-x-contain border-t border-ink/10 [scrollbar-width:none]">
...
export function CrossSection() {
  return (
    <section
      ...
      className="yard-xray relative isolate overflow-hidden bg-paper py-20 text-ink sm:py-28 lg:py-36"
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[38rem] opacity-40"
        style={{
          background:
            "radial-gradient(circle at 82% 16%, color-mix(in srgb, var(--accent) 16%, transparent), transparent 30%), linear-gradient(rgba(15,45,36,0.035) 1px, transparent 1px), linear-gradient(90deg, rgba(15,45,36,0.035) 1px, transparent 1px)",
          backgroundSize: "auto, 48px 48px, 48px 48px",
        }}
      />

      <div>
        <header className="grid items-end gap-10 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.55fr)] lg:gap-20">
          <div>
            <div className="flex items-center gap-4">
              <span className="font-mono text-[0.75rem] tracking-[0.22em] text-[color:var(--accent)] uppercase">
                Field study / 01
              </span>
              <span
                aria-hidden="true"
                className="h-px w-14 bg-[color:var(--accent)]/45"
              />
            </div>
            <h2
              id="ground-heading"
              className="mt-6 max-w-[13ch] font-display text-[clamp(2.8rem,7vw,7.6rem)] leading-[0.82] font-semibold tracking-[-0.065em] uppercase"
            >
              Great yards
              <span className="mt-2 block text-[color:var(--accent)]">
                run deep.
              </span>
            </h2>
          </div>

          <div className="border-t border-ink/15 pt-5 lg:mb-2">
            <div className="flex items-center justify-between gap-4 font-mono text-[0.75rem] tracking-[0.17em] text-ink-soft uppercase">
              <span>Surface → system</span>
              <span>12 services</span>
            </div>
            <p className="mt-6 max-w-md text-base leading-relaxed text-ink/70 sm:text-lg">
              What catches the eye is only half the work. We care for the
              visible yard and engineer the ground that keeps it thriving.
            </p>
          </div>
        </header>
